...
public class UAStateReceiver extends Callback {
		...
		try {
			...
			if (shouldAutoAnswer >= 200) {
				...
				if (pjService.mediaManager != null) {
					if (pjService.service.getGSMCallState() == TelephonyManager.CALL_STATE_IDLE && !hasOngoingSipCall) {
	...
	public void on_buddy_state(int buddyId) {
		...
		pjService.service.presenceMgr.changeBuddyState(PjSipService.pjStrToString(binfo.getUri()), binfo.getMonitor_pres(), presStatus, presStatusTxt);
		unlockCpu();
	}
	...
	public void on_pager(int callId, pj_str_t from, pj_str_t to, pj_str_t contact, pj_str_t mime_type, pj_str_t body) {
		...
		SipMessage msg = new SipMessage(canonicFromStr, toStr, contactStr, bodyStr, mimeStr, date, SipMessage.MESSAGE_TYPE_INBOX, fromStr);
	...
	public void on_pager_status(int callId, pj_str_t to, pj_str_t body, pjsip_status_code status, pj_str_t reason) {
		...
		int messageType = (status.equals(pjsip_status_code.PJSIP_SC_OK) || status.equals(pjsip_status_code.PJSIP_SC_ACCEPTED)) ? SipMessage.MESSAGE_TYPE_SENT : SipMessage.MESSAGE_TYPE_FAILED;
		...
		if (statusInt != SipCallSession.StatusCode.OK && statusInt != SipCallSession.StatusCode.ACCEPTED) {
			args.put(SipMessage.FIELD_BODY, bodyStr + " // " + reasonStr);
		}
		cr.update(SipMessage.MESSAGE_URI, args, SipMessage.FIELD_TO + "=? AND " + SipMessage.FIELD_BODY + "=? AND " + SipMessage.FIELD_TYPE + "=" + SipMessage.MESSAGE_TYPE_QUEUED, new String[] {
				toStr, bodyStr });
	...
	public void on_call_media_state(final int callId) {
		...
		try {
			 ...
			 */
			if (callInfo.getMediaStatus() == SipCallSession.MediaState.ACTIVE || callInfo.getMediaStatus() == SipCallSession.MediaState.REMOTE_HOLD) {
				...
				// Auto record
				if (mAutoRecordCalls && pjService.canRecord(callId) && !pjService.isRecording(callId)) {
	...
	public void reconfigure(Context ctxt) {
		mIntegrateWithCallLogs = SipConfigManager.getPreferenceBooleanValue(ctxt, SipConfigManager.INTEGRATE_WITH_CALLLOGS);
		mPreferedHeadsetAction = SipConfigManager.getPreferenceIntegerValue(ctxt, SipConfigManager.HEADSET_ACTION, SipConfigManager.HEADSET_ACTION_CLEAR_CALL);
		mAutoRecordCalls = SipConfigManager.getPreferenceBooleanValue(ctxt, SipConfigManager.AUTO_RECORD_CALLS);
		mMicroSource = SipConfigManager.getPreferenceIntegerValue(ctxt, SipConfigManager.MICRO_SOURCE);
	}
	...
	public boolean handleHeadsetButton() {
		...
		if (callInfo != null) {
			...
			if (callInfo.isIncoming() && (state == SipCallSession.InvState.INCOMING || state == SipCallSession.InvState.EARLY)) {
				if (pjService != null && pjService.service != null) {
					pjService.service.getExecutor().execute(new SipRunnable() {
						@Override
				...
				return true;
			} else if (state == SipCallSession.InvState.INCOMING || state == SipCallSession.InvState.EARLY || state == SipCallSession.InvState.CALLING || state == SipCallSession.InvState.CONFIRMED
