...
public final class SipManager {
	 ...
	 * {@link android.content.Context#startService(android.content.Intent intent)}
	...
	/**
	 * Action launched when user want to go in sip favorites. Should raise the
	 * sip favorites view.
	 */
	 ...
	 * By message here, we mean a SIP SIMPLE message of the sip simple protocol.
	 * Understand a chat / im message.
	 */
	 ...
	 * This is linked to the call record feature of CSipSimple available through
	 * {@link ISipService#startRecording(int)}
	 * <p>
	 * Provided extras :
	 * <ul>
	 * <li>{@link SipManager#EXTRA_FILE_PATH} the path to the recorded file</li>
	 * <li>{@link SipManager#EXTRA_CALL_INFO} the information on the call
	 * recorded</li>
	 * </ul>
	 * </p>
	 */
	...
	/**
	 * Broadcast to send when your activity doesn't allow anymore user to make
	 * outgoing calls.<br/>
	...
	/**
	 * Broadcast to send when you have launched a sip action (such as make
	 * call), but that your app will not anymore allow user to make outgoing
	 * calls actions.<br/>
	 * You have to pass registered {@link #EXTRA_OUTGOING_ACTIVITY}
	 * 
	 * @see #EXTRA_OUTGOING_ACTIVITY
	 ...
	 * You can expect {@link android.content.Intent#EXTRA_PHONE_NUMBER} as
	 * argument for the number to call. <br/>
	 * Your receiver must
	 * {@link android.content.BroadcastReceiver#getResultExtras(boolean)} with
	 * parameter true to fill response. <br/>
	 ...
	 * Any app that register this plugin and has rights to
	 * {@link #PERMISSION_USE_SIP} will appear in the call cards. <br/>
	 * The activity entry in manifest may have following metadata
	 * <ul>
	 * <li>{@link #EXTRA_SIP_CALL_MIN_STATE} minimum call state for this plugin
	 * to be active. Default {@link SipCallSession.InvState#EARLY}.</li>
	 * <li>{@link #EXTRA_SIP_CALL_MAX_STATE} maximum call state for this plugin
	 * to be active. Default {@link SipCallSession.InvState#CONFIRMED}.</li>
	 * <li>{@link #EXTRA_SIP_CALL_CALL_WAY} bitmask flag for selecting only one
	 * way. {@link #BITMASK_IN} for incoming; {@link #BITMASK_OUT} for outgoing.
	 * Default ({@link #BITMASK_IN} | {@link #BITMASK_OUT}) (any way).</li>
	 * </ul>
	 * Receiver activity will get an extra with key {@value #EXTRA_CALL_INFO}
	 * with a {@link SipCallSession}.
	 */
	 ...
	 * You can expect {@link android.content.Intent#EXTRA_PHONE_NUMBER} as
	 * argument for the number to rewrite. <br/>
	 * Your receiver must
	 * {@link android.content.BroadcastReceiver#getResultExtras(boolean)} with
	 * parameter true to fill response. <br/>
	 * Your response contains :
	 ...
	 */
	public static final Uri CALLLOG_URI = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + AUTHORITY + "/" + CALLLOGS_TABLE_NAME);
	/**
	 * Base uri for a specific call log. Should be appended with id of the call
	 * log.
	 */
	public static final Uri CALLLOG_ID_URI_BASE = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + AUTHORITY + "/" + CALLLOGS_TABLE_NAME + "/");
	 ...
	 */
	public static final Uri FILTER_URI = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + AUTHORITY + "/" + FILTERS_TABLE_NAME);
	/**
	 * Base uri for a specific filter. Should be appended with filter id.
	 */
	public static final Uri FILTER_ID_URI_BASE = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + AUTHORITY + "/" + FILTERS_TABLE_NAME + "/");
	...
	/**
	 * Tell sip service that it's an user interface requesting for outgoing
	 * call.<br/>
	 * It's an extra to add to sip service start as string representing unique
	 * key for your activity.<br/>
	 * We advise to use your own component name
	 * {@link android.content.ComponentName} to avoid collisions.<br/>
	 * Each activity is in charge unregistering broadcasting
	 * {@link #ACTION_OUTGOING_UNREGISTER} or
	 * {@link #ACTION_DEFER_OUTGOING_UNREGISTER}<br/>
	 * 
	 ...
	 * In case an account is specified in the outgoing call intent with
	 * {@link SipProfile#FIELD_ACC_ID} and the application doesn't find this
	 * account, this extra parameter allows to determine what is the fallback
	 * behavior of the activity. <br/>
	 * By default {@link #FALLBACK_ASK}. Other options :
	 */
	public static final String EXTRA_FALLBACK_BEHAVIOR = "fallback_behavior";
	/**
	 * Parameter for {@link #EXTRA_FALLBACK_BEHAVIOR}. Prompt user with other
	 * choices without calling automatically.
	 */
	public static final int FALLBACK_ASK = 0;
	/**
	 * Parameter for {@link #EXTRA_FALLBACK_BEHAVIOR}. Warn user about the fact
	 * current account not valid and exit. WARNING : not yet implemented, will
	 * behaves just like {@link #FALLBACK_ASK} for now
	 */
	public static final int FALLBACK_PREVENT = 1;
	/**
	 * Parameter for {@link #EXTRA_FALLBACK_BEHAVIOR} Automatically fallback to
	 * any other available account in case requested sip profile is not there.
	 */
